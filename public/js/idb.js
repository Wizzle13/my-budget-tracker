...
request.onsuccess = function(event) {
    ...
    // check if app is online, if yes run uploadBudget_Item() function to send all local db data to api
    if (navigator.onLine) {
      uploadBudget_Item();
    }
};
...
function saveRecord(transaction) {
    ...
    const record = db.transaction(['new_budget_item'], 'readwrite');
  
    // access the object store for `new_budget_item`
    const budget_itemObjectStore = transaction.objectStore('new_budget_item');
  
    // add transaction to your store with add method
    budget_itemObjectStore.add(record);
};

function uploadBudget_Item() {
    ...
    const getAll = budget_itemObjectStore.getAll();
    ...
    getAll.onsuccess = function() {
        ...
        budget_itemObjectStore.clear();

        alert('All saved budget_item has been submitted!');
        })
        .catch(err => {
        console.log(err);
        });                                                                                 
...
window.addEventListener('online', uploadBudget_Item);